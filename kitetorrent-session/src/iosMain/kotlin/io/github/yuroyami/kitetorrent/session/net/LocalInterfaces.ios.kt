 ...
 * stock `platform.posix` does not expose — wiring it up requires a small cinterop
 * `.def` (a build-config change), tracked as a follow-up. Until then we return just the
