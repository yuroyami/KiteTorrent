 ...
 * (and BEP-44 `get`/`put`) flow — the pure, network-free port of `node::generate_token`
 * and `node::verify_token` (`src/kademlia/node.cpp`).
 *
 ...
 * few minutes.
 *
 * **One deliberate deviation from libtorrent.** Upstream hashes `addr.address().to_string()`
 * — the textual form of the IP. `commonMain`'s [io.github.yuroyami.kitetorrent.peer.PeerAddress]
 ...
 * determinism and that it bind address+info-hash — both of which the byte form satisfies
 * at least as well as the string form. Tokens are opaque to every other node, so this
...
class DhtTokens(random: Random = Random.Default) {
     ...
     * either the current or the previous secret — port of `node::verify_token`. A token
     ...
     * drawn — port of the secret-rotation in `node::tick`
