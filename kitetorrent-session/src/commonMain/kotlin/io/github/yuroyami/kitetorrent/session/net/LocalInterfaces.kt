...
/**
 * One local network interface address — the slice of libtorrent's `ip_interface`
...
/**
 * Enumerate local interface addresses — port of `enum_net()` (enum_net.cpp). This is
 * a Tier-3 `expect`/`actual` because listing interfaces is a per-OS syscall with no
 * portable KMP API (`NetworkInterface` on JVM/Android, `getifaddrs` on Apple).
 */
